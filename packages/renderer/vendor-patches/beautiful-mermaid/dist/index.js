...
var SEQ = {
  ...
  dividerExtra: 24,
  /** Message label block dimensions */
  messageLabelPadX: 10,
  messageLabelPadY: 6,
  messageLabelGap: 8,
  messageLabelMinWidth: 40,
  messageLabelSideGap: 24,
  messageTextSafetyX: 48,
  messageTextSafetyRatio: 0.12,
  selfMessageLoopWidth: 30,
  selfMessageLabelGap: 8,
  /** Note dimensions */
  noteWidth: 60,
  notePadX: 14,
  notePadY: 10,
...
function measureSequenceTextBox(text, padX, padY, minWidth = 0, safetyX = 0, safetyRatio = 0) {
  const metrics = measureMultilineText(text, FONT_SIZES.edgeLabel, FONT_WEIGHTS.edgeLabel);
  const widthSafety = Math.max(safetyX, metrics.width * safetyRatio);
  return {
    textWidth: metrics.width,
    textHeight: metrics.height,
    width: Math.max(minWidth, metrics.width + widthSafety + padX * 2),
    height: metrics.height + padY * 2,
    lineHeight: metrics.lineHeight,
    lines: metrics.lines
  };
}
function measureSequenceMessageLabel(text) {
  return measureSequenceTextBox(text, SEQ.messageLabelPadX, SEQ.messageLabelPadY, SEQ.messageLabelMinWidth, SEQ.messageTextSafetyX, SEQ.messageTextSafetyRatio);
}
function measureSequenceNote(text) {
  return measureSequenceTextBox(text, SEQ.notePadX, SEQ.notePadY, SEQ.noteWidth, SEQ.noteTextSafetyX, SEQ.noteTextSafetyRatio);
}
function applySequenceActorGapRequirements(diagram, actorIndex, actorWidths, requiredGaps) {
  for (const msg of diagram.messages) {
    const fromIdx = actorIndex.get(msg.from);
    const toIdx = actorIndex.get(msg.to);
    if (fromIdx === void 0 || toIdx === void 0) continue;
    const label = measureSequenceMessageLabel(msg.label);
    if (fromIdx === toIdx) {
      if (fromIdx < actorWidths.length - 1) {
        const required = SEQ.selfMessageLoopWidth + SEQ.selfMessageLabelGap + label.width + SEQ.messageLabelSideGap;
        requiredGaps[fromIdx] = Math.max(requiredGaps[fromIdx] ?? 0, required);
      }
      continue;
    }
    ...
    const required = (label.width + SEQ.messageLabelSideGap * 2) / span;
    for (let i = minIdx; i < maxIdx; i++) requiredGaps[i] = Math.max(requiredGaps[i] ?? 0, required);
  }
  for (const note of diagram.notes) {
    const indexes = note.actorIds.map((id) => actorIndex.get(id)).filter((idx) => idx !== void 0);
    if (indexes.length === 0) continue;
    const minIdx = Math.min(...indexes);
    const maxIdx = Math.max(...indexes);
    const noteBox = measureSequenceNote(note.text);
    if (minIdx === maxIdx) {
      const actorIdx = minIdx;
      const actorHalfWidth = actorWidths[actorIdx] / 2;
      if (note.position === "left") {
        if (actorIdx > 0) {
          const required = actorHalfWidth + noteBox.width + SEQ.noteGap * 2;
          requiredGaps[actorIdx - 1] = Math.max(requiredGaps[actorIdx - 1] ?? 0, required);
        }
      } else if (note.position === "right") {
        if (actorIdx < actorWidths.length - 1) {
          const required = actorHalfWidth + noteBox.width + SEQ.noteGap * 2;
          requiredGaps[actorIdx] = Math.max(requiredGaps[actorIdx] ?? 0, required);
        }
      } else {
        const required = noteBox.width / 2 + SEQ.noteGap;
        if (actorIdx > 0) requiredGaps[actorIdx - 1] = Math.max(requiredGaps[actorIdx - 1] ?? 0, required);
        if (actorIdx < actorWidths.length - 1) requiredGaps[actorIdx] = Math.max(requiredGaps[actorIdx] ?? 0, required);
      }
      continue;
    }
    const span = Math.max(1, maxIdx - minIdx);
    const required = (noteBox.width + SEQ.noteGap * 2) / span;
    for (let i = minIdx; i < maxIdx; i++) requiredGaps[i] = Math.max(requiredGaps[i] ?? 0, required);
  }
  for (let i = 1; i < actorWidths.length; i++) {
    requiredGaps[i - 1] = Math.max(requiredGaps[i - 1] ?? 0, (actorWidths[i - 1] + actorWidths[i]) / 2 + 40);
  }
}
function layoutSequenceDiagram(diagram, _options = {}) {
  ...
  const requiredGaps = Array(Math.max(0, diagram.actors.length - 1)).fill(0);
  applySequenceActorGapRequirements(diagram, actorIndex, actorWidths, requiredGaps);
  const actorCenterX = [];
  let currentX = SEQ.padding + actorWidths[0] / 2;
  for (let i = 0; i < diagram.actors.length; i++) {
    if (i > 0) {
      const minGap = Math.max(SEQ.actorGap, requiredGaps[i - 1] ?? 0);
      currentX += minGap;
    }
  ...
  for (let msgIdx = 0; msgIdx < diagram.messages.length; msgIdx++) {
    ...
    const labelBox = measureSequenceMessageLabel(msg.label);
    if (isSelf) {
      messageY += Math.max(0, (labelBox.height - SEQ.selfMessageHeight) / 2);
    } else {
      messageY += labelBox.height + SEQ.messageLabelGap;
    }
    ...
    messageY += isSelf ? Math.max(SEQ.selfMessageHeight + SEQ.messageRowHeight, labelBox.height + SEQ.messageRowHeight) : SEQ.messageRowHeight;
    ...
    if (notesForMsg && notesForMsg.length > 0) {
      ...
      let noteY = messages[msgIdx].y + selfLoopExtra + SEQ.messageLabelGap;
      for (const note of notesForMsg) {
        const noteBox = measureSequenceNote(note.text);
        const noteW = noteBox.width;
        const noteH = noteBox.height;
  ...
  const blocks = diagram.blocks.map((block) => {
    ...
    const dividers = block.dividers.map((d) => {
      ...
      if (d.label && msg?.label) {
        ...
        const msgLabelW = msg.labelBox?.width ?? measureSequenceMessageLabel(msg.label).width;
        const msgLabelLeft = msg.isSelf ? msg.x1 + SEQ.selfMessageLoopWidth + SEQ.selfMessageLabelGap : (msg.x1 + msg.x2) / 2 - msgLabelW / 2;
  ...
  for (const m of messages) {
    if (m.isSelf && m.label) {
      const labelLeft = m.x1 + SEQ.selfMessageLoopWidth + SEQ.selfMessageLabelGap;
      const labelWidth = m.labelBox?.width ?? measureSequenceMessageLabel(m.label).width;
      globalMaxX = Math.max(globalMaxX, labelLeft + labelWidth + SEQ.messageLabelSideGap);
    }
  }
...
function renderSequenceTextBlock(text, boxX, boxY, box, rectClass, rectAttrs, textAttrs, padX, padY) {
  const rect = `<rect class="${rectClass}" x="${boxX}" y="${boxY}" width="${box.width}" height="${box.height}" rx="5" ry="5" ${rectAttrs} />`;
  const textEl = renderMultilineText(
    text,
    boxX + padX,
    boxY + box.height / 2,
    FONT_SIZES.edgeLabel,
...
function renderMessage(msg) {
  ...
  const labelBox = msg.labelBox || measureSequenceMessageLabel(msg.label);
  parts.push(
  ...
  if (msg.isSelf) {
    const loopW = SEQ.selfMessageLoopWidth;
    const loopH = 20;
    const labelPadding = SEQ.selfMessageLabelGap;
    parts.push(
    ...
    const boxX = msg.x1 + loopW + labelPadding;
    const boxY = msg.y + loopH / 2 - labelBox.height / 2;
    parts.push(
      "  " + renderSequenceTextBlock(
        msg.label,
        boxX,
        boxY,
        labelBox,
        "message-label-bg",
        `fill="${SEQ.labelFill}" stroke="${SEQ.labelStroke}" stroke-width="0.75" opacity="0.96"`,
        `font-size="${FONT_SIZES.edgeLabel}" text-anchor="start" font-weight="${FONT_WEIGHTS.edgeLabel}" fill="var(--_text-muted)"`,
        SEQ.messageLabelPadX,
        SEQ.messageLabelPadY
      )
    );
  } else {
    parts.push(
    ...
    const boxX = midX - labelBox.width / 2;
    const boxY = msg.y - SEQ.messageLabelGap - labelBox.height;
    parts.push(
      "  " + renderSequenceTextBlock(
        msg.label,
        boxX,
...
function renderNote(note) {
  ...
  <polygon points="${bodyPoints}" fill="${SEQ.noteFill}" stroke="${SEQ.noteStroke}" stroke-width="${STROKE_WIDTHS.innerBox}" />
  <polygon points="${x + w - foldSize},${y} ${x + w},${y + foldSize} ${x + w - foldSize},${y + foldSize}" fill="${SEQ.noteFoldFill}" stroke="${SEQ.noteStroke}" stroke-width="${STROKE_WIDTHS.innerBox}" />
  ${renderMultilineText(
    note.text,
    x + SEQ.notePadX,
    y + h / 2,
